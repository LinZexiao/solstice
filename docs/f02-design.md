...
| `SetShares` | write: `shares` / `payable` / `claimed_period` fold | `service_accrued` reset; residue burned (sent to f099) |
| `Claim` | write: `claimed_period` rows, `payable` | none |

The award path reads the streams block every block (the records live there).
...
AwardBlockReward(miner, penalty, gas_reward, win_count):
    if epoch >= next_transition_epoch: apply valid due calls; drop cancellation-stranded calls and emit events
...
* `B` (`total_burn_minted`) is the cumulative w0 award residual. `S` (`total_service_minted`) is gross explicit-stream accrual. Both are stored because each otherwise needs the full weight and reward history.
* `M`, what miners have received, is derived: `M = T - B - S`. It is not stored.

Each award adds `BR = miner + service + burn` exactly and bumps `T`, `B`, and `S` by their parts. Fold dust was already counted in `S`, so burning it changes no counter; f099's receipts exceed `B` by cumulative dust while `M = T - B - S` remains exact and all counters stay monotone. The counters account for FIL ever minted; the claim state accounts only for the slice still inside f02: at every epoch f02's balance covers `Σ (service_accrued - Σ claimed_period + Σ payable)` over streams and tombstones. Its terms are period-scoped but the invariant isn't; the fold just moves value between terms.
...
Gate-position correctness is the SWA's job: it must keep its own step counter rather than deriving position from f02's w2, which is stale while a write is queued (a check inside the hold re-fires a step it already fired) and, because w2's discrete steps bump up against its ceiling's continuous slope (`1 - w1`), the FIP's `(w2 - W2_BASE)/W2_STEP` stops returning an integer as w2 wanders off the 5% grid, so it doesn't provide the necessary stability.

## Events

The actor surface has five event kinds: queued write, cancelled write, applied write, dropped write, and one claim-payout event per non-zero recipient payment. Exact names, field schemas, and encoding belong to review unit 3. Applied-write emission must work both when an explicit mutating call drains the queue and when `AwardBlockReward` applies due writes on the implicit path.

Implementation follows the existing built-in-actor event pattern in `actors/*/src/emit.rs`: actor-local helper functions build kebab-case events with `EventBuilder`, index identity fields used for filtering, leave value fields unindexed, and propagate `build` or `emit_event` failures as `ActorError`. `MockRuntime` tests construct the same event with `EventBuilder` and register it through `expect_emitted_event`.
...
Only well-formed calls that have lost an admitted prerequisite are dropped. Payload decoding failures, non-canonical queued calls, and inconsistent persisted accounting are illegal state: the method aborts without mutating state, rather than misreporting corruption as governance cancellation fallout.

The slot key space bounds the queue at `3 * MAX_STREAMS + 2`: three per-stream operations and two schedule-wide operations.
